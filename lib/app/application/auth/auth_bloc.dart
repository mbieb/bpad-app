...
import 'package:bpad_app/app/domain/auth/auth_objects.dart';
  ...
  FutureOr<void> _onEvent(
    ...
    await event.map(
      ...
      fullNameChanged: (event) {
        emit(
          state.unmodified.copyWith.profileForm(fullName: FullName(event.fullName)),
        );
      },
      genderChanged: (event) {
        emit(
      ...
      birthDateChanged: (event) {
        emit(
          state.unmodified.copyWith.profileForm(birthDate: some(event.birthDate)),
        );
      },
      ...
      takePicture: (event) async {
        final failureOrPicture = await _authRepository.takePicture(event.imageSource);
        ...
        return failureOrPicture.fold(
          ...
          (picture) => emit(
            state.unmodified.copyWith(
              ...
              profileForm: state.profileForm.copyWith(imageUrl: optionOf(picture)),
              failureOrSuccessOption: some(right(const AuthSuccess.takePhotoSuccess())),
            ),
          ),
        );
      },
      updateProfile: (event) async {
        emit(state.loading);
        Option<Either<AppFailure<AuthFailure>, AuthSuccess>> failureOrSuccessOption = none();
        if (state.profileForm.failureOption.isNone()) {
          final response = await _authRepository.updateProfile(state.profileForm);
