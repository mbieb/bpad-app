...
class ValueValidators {
  ...
  static Either<ValueFailure<String>, String> validatePassword(
    ...
    int patternMatch = [hasAlphabet, hasNumeric, hasSpecialChar].where((element) => element).length;
    if (patternMatch == 1) {
  ...
  static Either<ValueFailure<String>, String?> validateStringCannotEmpty(String? input) {
  ...
  static Either<ValueFailure<String>, String> validateToken(String input, int maxLength) {
    if (input.length == maxLength) {
      return right(input);
  ...
  static Either<ValueFailure<String>, String> validateDate(DateTime input, String pattern) {
    String? sDate = CommonUtils.dateFormat(pattern, input);
    if (sDate == null) {
      return left(ValueFailure.invalidDateTime(failedValue: input, pattern: pattern));
    }
    return right(sDate);
  }
}
